...
class PermissionAdmin(admin.ModelAdmin):
    readonly_fields = ('name', 'content_type', 'codename', 'usuarios',
                       'grupos')
    ...
    def grupos(model, obj):
        grupos = "<br>".join([
            f'<a href="/admin/auth/group/{u.id}/change/"\
            target="_blank">{ u.name }</a>'
            for u in obj.group_set.all()])
        return format_html(grupos)

    def usuarios(model, obj):
        usuarios = "<br>".join([
            f'<a href="/admin/users/user/{u.id}/change/"\
            target="_blank">{ u.get_full_name() }</a>'
...
class UserAdmin(UserAdmin):
    ...
    readonly_fields = ('date_joined', 'token', 'last_login', 'created_by',
                       'updated_by', 'created_at', 'updated_at',
                       'is_gestor_espacos', 'is_admin_espacos',
                       'repositorios_vinculados')
    list_per_page = 10
    ...
    ordering = ['-first_name']
    actions = ['activeUser', 'generateToken', 'revokeToken',
               'renew_session', 'teste_gateway']

    def is_gestor_espacos(self, obj):
        espacos = "<br>".join([
            f'<a href="/admin/agenda/espaco/{u.id}/change/"\
            target="_blank">{ u.descricao }</a>'
            for u in obj.gestor_espaco_users.all()])
        return format_html(espacos)
    is_gestor_espacos.short_description = 'Gestor nos espaços'

    def repositorios_vinculados(self, obj):
        repositorios = "<br>".join([
            f'<a href="/admin/repositoriobi/repo/{u.id}/change/"\
            target="_blank">{ u.descricao }</a>'
            for u in obj.repo_set.all()])
    ...
    def is_admin_espacos(self, obj):
        espacos = "<br>".join([
            '<a href="/admin/agenda/espaco/{}/change/" target="_blank">{}</a>'
            .format(u.id, u.descricao) for u in obj.admin_users.all()])
        return format_html(espacos)
        ...
        for obj in queryset:
            if not obj.is_staff:
                ...
                if not obj.is_active:
                    obj.is_active = not obj.is_active
                    obj.save()
                    register_gateway(request, 'Reativando usuário', 'Users')

    activeUser.short_description = "Ativar/Desativar usuário"
    ...
    def renew_session(modeladmin, request, queryset):
        for obj in queryset:
            ...
            register_gateway(request, 'Renovando sessão do usuário', 'Users')
    renew_session.short_description = "renovar sessão do usuário"
    # TODO: test user not superuser
    ...
    def teste_gateway(modeladmin, request, queryset):
        for obj in queryset:
            register_gateway(
                request, f"Testando Gateway no usuário ws: {obj.username}",
                'Users')
    teste_gateway.short_description = "Testar Gateway"
    ...
    def generateToken(modeladmin, request, queryset):
        for obj in queryset:
            ...
            obj.save()
            register_gateway(
                request, f'Gerando Token para usuário {obj.username}', 'Users')
            messages.success(request, f'Geração de Token finalizada: {token}')
    generateToken.short_description = "Gerar Token de Usuário"
    ...
    def revokeToken(modeladmin, request, queryset):
        for obj in queryset:
            ...
            obj.save()
            register_gateway(
                request, f'Revogando token do usuário {obj.username}', 'Users')
            messages.success(request, 'Revogação de Token finalizada')
...
class ApiRequestsAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'view_basename', 'view_action',
                    'view_method', 'requested_at')
...
class GroupsAdmin(GroupAdmin):
    ...
    def usuarios(model, obj):
        to_html = "<br>".join([
            '<a href="/admin/users/user/{}/change/" target="_blank">{}</a>'
            .format(
                u.id,
                u.get_full_name()) for u in obj.user_set.all()])
        usuarios = to_html
        return format_html(usuarios)
