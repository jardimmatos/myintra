...
class TimeStampedModel(models.Model):
    ...
    created_at = models.DateTimeField(_('criado em'), auto_now_add=True, auto_now=False, null=True)
    updated_at = models.DateTimeField(_('alterado em'), auto_now_add=False, auto_now=True, null=True)
...
class TeamsChannel(TimeStampedModel):
    titulo = models.CharField(max_length=255, blank=False, null=True, verbose_name="Título do canal")
    webhook= models.URLField(unique=True, blank=False, null=True, verbose_name="Webhook", max_length=255)
...
class Arquivo(TimeStampedModel):
    arquivo = models.FileField(upload_to='arquivos', null=True, blank=True, verbose_name='Arquivos')
...
class MonitorServico(TimeStampedModel):
    servico = models.CharField(max_length=250, verbose_name='Serviço', blank=False, null=True)
    url = models.CharField(max_length=250, verbose_name='URL', blank=False, null=True)
    referencia = models.CharField(max_length=250, verbose_name='Referência', blank=True, null=True)
...
class GlobalSetting(TimeStampedModel):
    description = models.CharField(max_length=100, default=_("Configuração global padrão"), null=True, blank=False, verbose_name=_("Descrição"))
    logo = ResizedImageField(size=[100, 100], upload_to=path_logo, blank=True, null=True, verbose_name="Logo")
    ...
    def clean(self):
        ...
        if setting > 0:
            raise ValidationError(_("Já existe uma configuração global registrada no sistema!"))

    class Meta:
        verbose_name = 'Sistema - Configuração global'
        verbose_name_plural = 'Sistema - Configurações globais'
        permissions = (
            ('can_resetpassword_global', 'Redefinir senha Global'),
            ('can_gerenciar_containers', 'Pode Gerenciar Containers na Infraestrutura'),
        )
...
class SystemParams(TimeStampedIDModel):
    ...
    chave = models.CharField(max_length=80, null=True, blank=False, unique=True)
