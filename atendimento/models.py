...
class Unidade(TimeStampedIDModel):
    nome = models.CharField(max_length=255, unique=True, null=True,
                            blank=False)
...
class Atendente(TimeStampedIDModel):
    ...
    def clean(self):
        if self.local and self.local.unidade != self.unidade:
            raise ValidationError({'local': "Unidade do Local é diferente da \
                                   Unidade do Atendente!"})

        if self.prioridade and self.prioridade.unidade != self.unidade:
            raise ValidationError({'prioridade': "Unidade da Prioridade é \
                                   diferente da Unidade do Atendente!"})

        if self.perfil and self.perfil.unidade != self.unidade:
            raise ValidationError(
                {'perfil': "Unidade do Perfil é diferente da Unidade \
                 do Atendente!"})
    ...
    def alterar_numero_local(self, args):
        ...
        if 'prioridade' in args:
            try:
                self.prioridade = Prioridade.objects.get(
                    pk=args.get('prioridade'))
            except Exception:
                raise ValidationError(
                    "Prioridade não informada para o atendente!")
...
class PainelSenha(TimeStampedIDModel):
    ...
    atendimento = models.OneToOneField(
        'Atendimento', on_delete=models.CASCADE, null=True, blank=False)
    ...
    def chamar_senha_painel(self):
        # Valida se o status do atendimento é Chamado(status_atendimento
        # alterado antes de ser chamado no painel)
        self.atendimento.target_before_status_validation(
        ...
        historicos = [{
            ...
            'local': "{} {}".format(
                h.atendimento.atendente.local.nome,
                h.atendimento.atendente.numero_local
            ),
        ...
        obj = {
            ...
            'local': "{} {}".format(
                self.atendimento.atendente.local.nome,
                self.atendimento.atendente.numero_local
            ),
            'historico': historicos
        }
        call_ws(f'room_channel_painel_atendimento_{unidade_id}', '',
                'CALL-TICKET', obj, notify=True)
        return obj
...
class Atendimento(TimeStampedIDModel):
    unidade = models.ForeignKey(
        Unidade, on_delete=models.CASCADE, null=True, blank=False,
        verbose_name="Unidade de Atendimento",
        help_text="Unidade à qual o atendimento pertence")
    cliente = models.ForeignKey(
        Cliente, on_delete=models.PROTECT, null=True, blank=True,
        help_text="Cliente identificado para o Atendimento")
    prioridade = models.ForeignKey(
        Prioridade, on_delete=models.CASCADE, null=True, blank=False,
        help_text="Prioridade definida para o Atendimento")

    # incremental pelo serviço do atendimento
    senha = models.IntegerField(
        null=True, blank=False, help_text="Sequência de senha do dia/serviço")

    servico = models.ForeignKey(
        Servico, on_delete=models.CASCADE, null=True, blank=False,
        verbose_name="Serviço", help_text="Serviço definido na triagem de \
            senha. Se redirecionado, este serviço pode ser alterado")
    atendente_tri = models.ForeignKey(
        Atendente, on_delete=models.CASCADE, null=True, blank=False,
        related_name='atendimento_usuario_tri', verbose_name="Triagem",
        help_text="Usuário que realizou a triagem da senha")
    redirecionado_por = models.ForeignKey(
        Atendente, on_delete=models.CASCADE, null=True, blank=True,
        related_name='atendimento_redirecionado_por',
        verbose_name="Redirecionador",
        help_text="Usuário que realizou o redirecionamento para outro serviço")
    servico_original = models.ForeignKey(
        Servico, on_delete=models.CASCADE, null=True, blank=True,
        related_name='servico_original_atendimento',
        help_text="Serviço original definido antes de qualquer \
            redirecionamento")

    data_chegada = models.DateTimeField(
        auto_now_add=False, auto_now=False, null=True, blank=True,
        verbose_name="Chegada", help_text="Data/Hora definida no ato da \
            triagem da senha")
    data_chamada = models.DateTimeField(
        auto_now_add=False, auto_now=False, null=True, blank=True,
        verbose_name="Chamada", help_text="Data/Hora definida no ato da \
            chamada de senha no Painel")
    data_inicio = models.DateTimeField(
        auto_now_add=False, auto_now=False, null=True, blank=True,
        verbose_name="Início", help_text="Data/Hora definida no ato do \
            início do atendimento")
    data_fim = models.DateTimeField(
        auto_now_add=False, auto_now=False, null=True, blank=True,
        verbose_name="Encerramento", help_text="Data/Hora definida no ato \
            do encerramento do atendimento")

    # Campos nulos no ato da triagem, preenchidos somente na chamada de senha
    atendente = models.ForeignKey(
        Atendente, on_delete=models.CASCADE, null=True, blank=True,
        related_name='atendimento_atendente',
        help_text="Atendente responsável pelo atendimento")
    local = models.ForeignKey(
        Local, on_delete=models.CASCADE, null=True, blank=True,
        help_text="Local de Atendimento parametrizado para o usuário \
            atendente")

    # diferenca entre (data_chegada - data_chamada)
    tempo_espera = models.IntegerField(
        default=None, null=True, blank=True, verbose_name="Espera",
        help_text="Tempo de espera em \
            segundos")  # atualizar na chamada da senha

    # diferenca entre (data_chamada - data_inicio)
    tempo_deslocamento = models.IntegerField(
        default=None, null=True, blank=True, verbose_name="Deslocamento",
        help_text="Tempo entre a chamada de senha no painel e início do \
            atendimento")  # atualiza no início do atendimento

    # diferenca entre (data_inicio - data_fim)
    tempo_atendimento = models.IntegerField(
        default=None, null=True, blank=True,
        help_text="Tempo entre o início e o encerramento do \
            atendimento")  # atualiza no término do atendimento

    # soma do tempo total
    # (tempo_espera + tempo_atendimento + tempo_deslocamento )
    tempo_permanencia = models.IntegerField(
        default=None, null=True, blank=True, verbose_name="Permanência",
        help_text="Tempo total desde a triagem de senha até o tempo de \
            encerramento")  # atualiza no término do atendimento

    avaliacao = models.IntegerField(
        default=None, null=True, blank=True, verbose_name="Nota",
        help_text="Nota de avaliação do atendimento")

    status_atendimento = models.CharField(
        max_length=50, default=enums.StatusAtendimento.emitido.name,
        choices=[(s.name, s.value) for s in enums.StatusAtendimento],
        null=True, blank=False, verbose_name="Status",
    ...
    resolucao = models.CharField(
        ...
        choices=[(r.name, r.value) for r in enums.ResolucaoAtendimento],
        null=True, blank=True, verbose_name="Resolução",
        help_text="Estado de Resolução do atendimento")
    obs = models.TextField(
        null=True, blank=True, verbose_name="Observação",
        help_text="Informações adicionais sobre o atendimento além das \
            observações comentadas")

    historico = models.BooleanField(
        default=False, verbose_name="Histórico",
        help_text="Os Atendimentos são definidos como históricos quando \
            foram finalizados como encerrados ou não comparecidos do dia \
                anterior. Para os demais atendimentos(do dia anterior), \
                    é necessário encerrá-los")
    ...
    def triagem_senha(self, args):
        with transaction.atomic():
            ...
            if 'unidade' not in args:
                raise ValidationError(
                    "Unidade não informado!")
            try:
                self.unidade = Unidade.objects.get(
                    pk=args.get('unidade'))
            except Exception:
                raise ValidationError(
                    "Falha ao selecionar Unidade!")

            if 'servico' not in args:
                raise ValidationError(
                    "Serviço não informado!")
            try:
                self.servico = Servico.objects.get(pk=args.get('servico'))
                self.servico_original = Servico.objects.get(
                    pk=args.get('servico'))
            except Exception:
                raise ValidationError(
                    "Falha ao selecionar Serviço/Departamento!")

            if 'prioridade' not in args:
                raise ValidationError(
            ...
            try:
                ...
                raise ValidationError(
                    "Falha ao selecionar prioridade!")

            self.sigla = self.servico.sigla

            if 'atendente_tri' not in args:
                raise ValidationError(
                    "Usuário de triagem Não informado!")
            try:
                self.atendente_tri = Atendente.objects.get(
                    pk=args.get('atendente_tri'))
            except Exception:
                raise ValidationError(
                    "Falha ao atribuir usuário na triagem de senha!")
            try:
    ...
    def redirecionar_atendimento(self, args):
        ...
        self.target_before_status_validation([
            enums.StatusAtendimento.chamado.name,
        ...
        if not self.data_chamada or True:
            with transaction.atomic():
                ...
                try:
                    ...
                    print(ex)
                    raise ValidationError(
                        "Falha ao calcular tempo de espera!")

                try:
                    # Disparar Senha
                    painel, _ = PainelSenha.objects.get_or_create(
                        atendimento=self)
                    if 'silent' in args:
                    ...
                    print(ex)
                    raise ValidationError("Falha ao criar chamada de Painel!")
        return Atendimento.objects.get(id=self.id)

    def iniciar_atendimento(self):
        if self.status_atendimento == enums.StatusAtendimento.iniciado.name:
            raise ValidationError("Atendimento já foi iniciado!")
    ...
    def encerrar_atendimento(self, args):
        ...
        if 'resolucao' in args:
            self.resolucao = enums.ResolucaoAtendimento[
                args.get('resolucao')].name
        else:
            raise ValidationError(
                "Resolução não informada para o atendimento!")
        ...
        at.tempo_permanencia = at.tempo_espera \
            + at.tempo_atendimento + at.tempo_deslocamento
        at.save()
    ...
    def target_before_status_validation(self, target_before=[]):
        """
            Valida qual o status anterior é necessário para o atendimento
            precisar prosseguir com o fluxo
        """
        if self.status_atendimento in target_before:
            ...
            raise ValidationError(
                "Atendimento foi chamado. Atendimento precisar ser \
                    Iniciado ou definido como \"Não compareceu\"")

        elif self.status_atendimento == enums.StatusAtendimento.iniciado.name:
            raise ValidationError(
                "Senha do atendimento precisar ser encerrado!")

        elif (self.status_atendimento in
              [enums.StatusAtendimento.encerrado.name]):
            raise ValidationError("Atendimento já foi encerrado!")
    ...
    def senhas_por_servicos_do_atendente(self, unidade_id, atendente_id):
        ...
        atendimentos = Atendimento.objects \
            ...
            .filter(servico__id__in=atendente.servicos.values_list('id',
                                                                   flat=True))
        return atendimentos

    def to_label_prioridade(self):
        try:
            return self.prioridade.nome
        except Exception:
            return self.prioridade
    ...
    def to_label_atendente(self):
        try:
            return self.atendente.usuario.username
        except Exception:
