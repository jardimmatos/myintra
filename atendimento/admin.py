...
class ServicoAdmin(AbstractAtendimentoAdmin):
    ...
    list_display = ('id','unidade', 'nome', 'descricao', 'departamento', 'sigla','ativo')
    autocomplete_fields = ('unidade',)
...
class AtendenteAdmin(AbstractAtendimentoAdmin):
    list_select_related = ('unidade','usuario', 'perfil', 'local', 'prioridade')
    list_prefetch_related = ('servicos',)
    list_display = ('usuario', 'to_label_unidade','to_label_local', 'numero_local', 'to_label_perfil')
...
class AtendimentoAdmin(AbstractAtendimentoAdmin):
    list_select_related = ('unidade', 'cliente', 'prioridade', 'servico', 'local', 'atendente')
    list_per_page = 50
    list_display = (
        'id', 'unidade', 'status_atendimento', 'historico', 'cliente', 
        'sigla_senha', 
        'to_label_prioridade', 'to_label_servico', 'to_label_local', 'atendente',
        'data_chegada', 'data_chamada', 'data_inicio', 'data_fim',
        'tempo_espera', 'tempo_deslocamento', 'tempo_atendimento', 'tempo_permanencia'
        )
    readonly_fields = (
                    ...
                    'tempo_espera', 'tempo_deslocamento', 'tempo_atendimento', 'tempo_permanencia',
                    'status_atendimento', 'sigla_senha', 'historico', 'avaliacao', 'comentarios',
                    'created_at', 'updated_at', 'created_by', 'updated_by',
                    )
    search_fields = ('sigla_senha', 'cliente__matricula', 'cliente__nome', 'atendente__usuario__username')

    # Incluir atendente, encarece as consultas de filtros
    list_filter = ('unidade','historico','prioridade','status_atendimento', 'servico',)
    date_hierarchy = 'data_chegada'

    def comentarios(self, obj):
        comentarios = "<br>".join([ f'<span>Em {u.get_created_at}, {u.created_by.first_name} observou: { u.comentario }</span>' for u in obj.atendimentocomentario_set.all()])
...
class PainelSenhaAdmin(AbstractAtendimentoAdmin):
    list_display = ('id', 'ref_atendimento', 'sigla_senha','atendimento', 'created_at')
    # readonly_fields = ('created_at', 'updated_at', 'created_by', 'updated_by')
