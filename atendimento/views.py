...
class Painel(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['host_backend'] = 'localhost:8000' if settings.DEBUG else 'meuservidor.dev.br'
        return context
...
class Acompanhamento(DetailView, UpdateView):
    ...
    def get_object(self, queryset=None):
        try:
            ...
            # Renderize um template alternativo quando o objeto não for encontrado
            return None
...
class AtendimentoViewSet(viewsets.ViewSet):
    ...
    def get_authenticators(self):
        # não exigir authentication_classes para a view de unidades, que é utilizada no painel (independerá de autenticação)
    ...
    def get_permissions(self):
        # não exigir permissão para a view de unidades, que é utilizada no painel (independerá de autenticação)
    ...
    def list_servicos(self, request):
        ...
        """
        serial = request.query_params.get('serial', enums.TypeSerials.simple.name)
        ...
        try:
            ...
            if serial == enums.TypeSerials.simple.name:
                serializer = serializer_apps.ServicoSimpleSerializer(servicos, many=True)
            else:
                serializer = serializer_apps.ServicoSerializer(servicos, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def list_prioridades(self, request):
        ...
        try:
            ...
            serializer = serializer_apps.PrioridadeSerializer(prioridades, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as ex:
            return Response({
    ...
    def triagem_senha(self, request):
        ...
        try:
            ...
            self.fila_senhas(obj.unidade.id, enums.OriginCallFilaSenha.triagem_senha.name)
    ...
    def proxima_senha(self, request):
        ...
        args = request.data
        try:
            atendente = models.Atendente.objects.get(usuario=request.user, ativo=True)
        except models.Atendente.DoesNotExist:
            return Response({
                "error": "Atendente ativo não disponível para chamada de senha!"
        ...
        serializer = serializer_apps.PainelSenhaSerializer(atendimento.painelsenha, many=False)
        # Atualiza lista de senhas no frontend
        self.fila_senhas(atendimento.unidade.id, enums.OriginCallFilaSenha.proxima_senha.name)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def chamar_senha_novamente(self, request, pk):
        ...
        except models.PainelSenha.DoesNotExist as ex:
            return Response({
    ...
    def iniciar_atendimento(self, request, pk):
        ...
        try:
            ...
            painel = models.PainelSenha.objects.get(pk=pk)
            serializer = serializer_apps.PainelSenhaSerializer(painel, many=False)
            return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def redirecionar_atendimento(self, request, pk):
        """
            ...
            pk = painel.id
            payload: {
                servico: ID (ID do servico para o qual está sendo redirecionado)
            }
        """
        args = request.data
        try:
            with transaction.atomic():
                painel = models.PainelSenha.objects.get(pk = pk)
                atendimento = models.Atendimento.objects.get(pk=painel.atendimento.pk)
                atendimento.redirecionar_atendimento(args)
                painel.delete()
                self.fila_senhas(atendimento.unidade.id, enums.OriginCallFilaSenha.redirecionar.name)
        except models.PainelSenha.DoesNotExist:
            return Response({ "error": "Atendimento não encontrado!" }, status=status.HTTP_404_NOT_FOUND)
        except Exception as ex:
            return Response({ "error": "Falha ao redirecionar atendimento! Erro: "+str(ex) }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def encerrar_atendimento(self, request, pk):
        ...
        try:
            ...
            painel = models.PainelSenha.objects.get(pk=pk)
            # Atualiza lista de senhas no frontend
            self.fila_senhas(painel.atendimento.unidade.id, enums.OriginCallFilaSenha.encerrar.name)
            # serializer = serializer_apps.PainelSenhaSerializer(painel, many=False)
            return Response(status=status.HTTP_200_OK)
        except Exception as ex:
            return Response({
    ...
    def get_painel_atual_atendente(self, request):
        """ Recupera o atendente a partir de um atendimento do usuário que já foi chamado ou iniciado (que está em andamento)"""
        try:
            painel_do_atendente = models.PainelSenha.objects.get(
                ...
                atendimento__status_atendimento__in=[
                    ...
                    ])#.order_by('atendimento__data_chegada')
            serializer = serializer_apps.PainelSenhaSerializer(painel_do_atendente, many=False)
            # Atualiza lista de senhas no frontend
            self.fila_senhas(request.user.atendente.unidade.id, enums.OriginCallFilaSenha.atual_atendente.name)
        except models.PainelSenha.DoesNotExist:
    ...
    def get_atendente_logado(self, request):
        try:
            atendente = models.Atendente.objects.get(usuario=request.user, ativo=True)
            serializer = serializer_apps.AtendenteSerializer(atendente, many=False)
            # Atualiza lista de senhas no frontend
            self.fila_senhas(atendente.unidade.id, enums.OriginCallFilaSenha.atendente_logado.name)
        except models.Atendente.DoesNotExist:
            return Response({
    ...
    def alterar_atendente(self, request):
        try:
            ...
            if local:
                try:
                    l = models.Local.objects.get(pk=local)
                    atendente.local = l
    ...
    def listar_comentarios(self, request, pk):
        """ pk: Atendimento"""
        comentarios = models.AtendimentoComentario.objects.filter(atendimento=pk)
        serializer = serializer_apps.AtendimentoComentarioSerializer(comentarios, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def fila_senhas(self, unidade_id, origin_trigger):
        """ Execução do Celery necessária """
        tasks.atendimento_fila_senhas.delay({'unidade_id': unidade_id, 'origin': origin_trigger })

    def triagem_notifica_atendente(self, unidade_id, servico_id):
        """ Execução do Celery necessária """
        tasks.atendimento_triagem_notifica_atendente.delay({'unidade_id': unidade_id, 'servico_id': servico_id })
    ...
    def executa_chamada_manual(self, args):
        ...
        try:
            ...
            raise drf_serializers.ValidationError('Atendimento não encontrado')
        except Exception as ex:
            print(ex)
            raise drf_serializers.ValidationError('Falha ao chamar a senha manualmente!'+str(ex))

        return atendimento

    def executa_chamada_automatica(self, args):
        atendente = args.get('atendente')
        peso_convencional = models.Prioridade.objects.filter(nome__in=['Convencional','Normal']).first().peso or 1
        peso_prioridade = models.Prioridade.objects.filter(nome='Prioridade').first().peso or 2

        existe_atendimento_aberto_do_usuario = models.Atendimento.objects.filter(
                                            atendente=atendente,
                                            status_atendimento__in=[
                                                enums.StatusAtendimento.chamado.name, 
                                                enums.StatusAtendimento.iniciado.name
        ...
        if existe_atendimento_aberto_do_usuario:
            raise drf_serializers.ValidationError('Existe atendimento em andamento!')
        ...
        existe_prioridade_length = [a.id for a in atendimento[:limite_convencionais] if a.prioridade.peso == peso_prioridade]
        if len(existe_prioridade_length):
            atendimento = atendimento.filter(id=existe_prioridade_length[0]).first()
        else:
            atendimento = atendimento.first()
        try:
            atendimento = atendimento.proxima_senha(args)
        except Exception as ex:
            raise drf_serializers.ValidationError("Falha ao chamar senha!"+str(ex))
        return atendimento
