...
class WikiViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(manual_parameters=[
        ...
        openapi.Parameter(
            'q', openapi.IN_QUERY,
            description='Filtrar palavra em títulos e corpo das documentações',
            type=openapi.TYPE_STRING),
    ])
    def list(self, request):
        qs = self.get_queryset()
        if not request.user.is_superuser:
            # docs que eu criei (privados ou não), inclusive Docs de outros
            # setores (aos quais eu não pertenco)
            ...
            qs3 = qs.annotate(num_membros=Count('membros'))\
                .filter(num_membros__lte=0, ativo=True)
        ...
        if query:
            qs = qs.filter(
                Q(titulo__icontains=query) | Q(html__icontains=query))
